...
class Amount extends ValueObject {
  ...
  Amount.fromString(String value, {mustBeGreaterThanZero: false}) {
    ...
    _value = isValid ? double.parse(value) : (_mustBeGreaterThanZero ? 1.0 : 0.0);
  }
  ...
  void _applyDoubleContracts(double value) {
    addNotifications(Contract().isNotNull(value, 'Amount.value', 'O valor não pode ser nulo'));
    if (_mustBeGreaterThanZero)
      addNotifications(Contract().isGreatherThan(value, 0, 'Amount.value', 'O valor precisa ser maior do que zero'));
  }
  ...
  void _applyStringContracts(String value) {
    ...
    if (_mustBeGreaterThanZero) {
      ...
      addNotifications(Contract().isGreatherThan(newValue, 0, 'Amount.value', 'O valor deve ser maior do que zero'));
    }
  }
