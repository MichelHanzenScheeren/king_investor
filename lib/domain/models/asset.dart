...
class Asset extends Model {
  ...
  late Category _category; // ação, fii, stock, reit ("securityType")
  ...
  late String _walletForeignKey;
  late Amount _sales; // Lucro/prejuízo com vendas
  late Amount _incomes; // Rendimentos/provendos (dividendos, jcp)
  ...
  void setCategory(Category? category) {
    clearNotifications();
    _applyContracts(category);
    if (isValid) _category = category!;
  }

  void _applyContracts(Category? category) {
    addNotifications(Contract().requires().isNotNull(category, 'Asset.category', 'A categoria não pode ser nula'));
  }

  void registerBuy(Quantity quantity, Amount amount) {
    clearNotifications();
    if (!quantity.isValid) addNotification('Asset.registerBuy', 'Tentativa de operação com quantidade inválida');
    if (!amount.isValid) addNotification('Asset.registerBuy', 'Tentativa de operação com preço inválido');
    if (isValid) {
  ...
  void registerSale(Quantity quantity, Amount amount) {
    clearNotifications();
    if (!quantity.isValid) addNotification('Asset.registerSale', 'Tentativa de operação com quantidade inválida');
    if (!amount.isValid) addNotification('Asset.registerSale', 'Tentativa de operação com preço inválido');
